...
namespace driver {
    ...
    public:
        ...
        class iterator: public std::iterator<std::forward_iterator_tag, const Out&, std::ptrdiff_t, const Out*, const Out&> {
            friend class Adapter;

        public:
            Out operator*() const {
                return _adapter->_func(*_iterator);
            }
        ...
        Adapter(const In* in, Func func) :
            _in(in),
            _func(func) {
        }

        iterator begin() const {
            return iterator(this, std::move(_in->begin()));
        }

        iterator end() const {
            return iterator(this, std::move(_in->end()));
        }

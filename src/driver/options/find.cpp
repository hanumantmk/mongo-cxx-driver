...
void find::limit(std::int32_t limit) { _limit = limit; }

void find::max_time_ms(std::int64_t max_time_ms) { _max_time_ms = max_time_ms; }

void find::modifiers(bson::document::view modifiers) { _modifiers = modifiers; }

void find::no_cursor_timeout(bool no_cursor_timeout) { _no_cursor_timeout = no_cursor_timeout; }

void find::oplog_replay(bool oplog_replay) { _oplog_replay = oplog_replay; }

void find::projection(bson::document::view projection) { _projection = projection; }

void find::read_preference(class read_preference rp) { _read_preference = std::move(rp); }

void find::skip(std::int32_t skip) { _skip = skip; }
...
const optional<bool>& find::no_cursor_timeout() const { return _no_cursor_timeout; }

const optional<bool>& find::oplog_replay() const { return _oplog_replay; }
